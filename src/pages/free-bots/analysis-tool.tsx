...
const AnalysisTool: React.FC = () => {
    ...
    // --- Single volatility selection (not a multi-scan anymore) -------------
    ...
    // Per signal-key (symbol+kind) flag: true means "the NEXT trade on this
    // signal should hedge (take the opposite side)" because the last one lost.
    const hedgeActiveRef = useRef<Record<string, boolean>>({});
    // Per signal-key flag so the Place Trade button disables while a contract
    // from that same signal is still open, instead of allowing a stacked re-fire.
    ...
    // --- Trading (manual only — fired by the "Place Trade" button) ---------
    ...
    /**
     * Sends the buy request for a signal the user explicitly chose to trade,
     * then subscribes to the contract until it settles so we know whether to
     * grow the martingale stake and/or flip to the hedge side next time.
     */
    const fireTrade = useCallback((signal: AnySignal, label: string) => {
        if (!api_base.api) return;

        const key = signalKey(signal);
        if (pendingRef.current[key]) return; // a trade from this signal is still open
        ...
        api_base.api
            ...
            .then((res: any) => {
                ...
                // martingale stake and hedge flag for the NEXT manual trade.
                const sub = api_base.api.onMessage().subscribe((response: any) => {
                    ...
                    // Take profit / stop loss check — stops the session, never places a trade automatically.
    ...
    // --- Tick subscription (one symbol at a time) ---------------------------
    ...
    // Recompute signals on an interval while scanning is on. This ONLY reads
    // ticks and computes stats — it never places a trade by itself.
    useEffect(() => {
        if (!scanning) {
            setSignals(null);
            return undefined;
